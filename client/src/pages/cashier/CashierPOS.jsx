  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <div className="p-5 text-sm" id="invoice-slip">
          <div className="text-center mb-4">
            <h3 className="font-bold text-lg text-gray-800">POS System</h3>
            <p className="text-gray-500 text-xs">
              {new Date(sale.createdAt || Date.now()).toLocaleString("en-PK")}
            </p>
            <p className="text-indigo-600 font-medium mt-1">
              Invoice #: {sale.saleNumber || sale.invoiceNumber || sale._id}
            </p>
          </div>
            ...
            {sale.paymentMethod === "cash" && (
              <>
                ...
                <div className="flex justify-between text-indigo-600 font-medium">
                  <span>Change</span>
                  <span>Rs. {sale.changeReturn}</span>
                </div>
              </>
            )}
          </div>
...
const CashierPOS = () => {
  ...
  const incQty = (id) =>
    setCart((prev) =>
      prev.map((i) =>
        i.productId === id && i.quantity < i.maxStock
          ? { ...i, quantity: i.quantity + 1, total: (i.quantity + 1) * i.salePrice }
          : i
      )
    );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const savedInvoice = response.sale || response.data || response;
      ...
      <div className="w-80 xl:w-96 bg-white border-l border-gray-100 flex flex-col shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <CartItem key={item.productId} item={item} onInc={incQty} onDec={decQty} onRemove={removeItem} />
            ))
          )}
        </div>
        ...
        <div className="border-t border-gray-100 p-4 space-y-3">
          ...
          <div className="bg-gray-50 rounded-xl p-3 space-y-1.5 text-sm">
            ...
            {discountAmt > 0 && (
              <div className="flex justify-between text-green-600">
                <span>
                  Discount ({appliedCoupon?.type === "percent" ? `${appliedCoupon.value}%` : "flat"})
                </span>
